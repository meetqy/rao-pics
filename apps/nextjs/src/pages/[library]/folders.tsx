...
const Page: NextPage = () => {
  ...
  return (
    <Layout navbar={<Header />} href="/folders" loadMoreContent={<span className="text-base-300 text-sm">{language.noData}</span>}>
      <>
        {assetsUrl && (
          <div className="grid grid-cols-2 gap-4 p-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8">
            {data?.map((item) =>
              item._count.images ? (
                <Link
                  href={`/${libraryName}?folder=${item.name}`}
                  key={item.id}
                  className="card glass relative overflow-hidden shadow-none transition-all hover:-top-0.5 hover:shadow-md"
                >
                  ...
                  <button className="btn btn-square btn-sm btn-circle bg-primary/30 text-primary-content absolute left-1 top-1 z-10 border-none font-mono font-normal backdrop-blur">
                    {item._count.images}
                  </button>
                </Link>
              ) : null,
