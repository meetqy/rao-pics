...
import { Button, Col, Divider, Layout, Menu, MenuProps, Row, theme, Typography } from "antd";
...
const treeRecursion = (data: ItemData[]) => {
  return data.map((item) => {
    return getItem(
      ...
      item.route,
      item.icon,
      item.children && item.children.length > 0 ? treeRecursion(item.children) : null,
...
const SiderMenu = () => {
  ...
  const itemsData = useMemo(() => {
          ...
          return {
            ...
            children: children.length
              ? children.map((childFolder) => ({
                  ...
                  icon: <FolderFilled style={{ color: childFolder.iconColor }} />,
                }))
              : null,
            onTitleClick: (info) => {
    ...
    return original;
  }, [counts, foldersTree, openKeys, onFolderIconClick, folders.length, router]);

  // 当前的folder
  const folder = useMemo(() => folders.find((item) => `/folder/${item.id}` === route), [route, folders]);

  // 当前的菜单信息
  const nowItemData = useMemo(() => itemsData.find((item) => item.route === route), [itemsData, route]);

  // 设置右侧菜单信息
  useEffect(() => {
    if (folder) {
      if (!route.includes(folder.id)) return;
      if (folder.name === rightBasic.name && folder._count.images === rightBasic.fileCount) return;
      ...
      if (nowItemData.title === rightBasic.name && nowItemData.count === rightBasic.fileCount) return;
    ...
    <Layout style={{ minHeight: "100vh" }}>
      ...
      >
        <Row>
          ...
          <Col span={2}></Col>
          <Col>
            <Link href={"https://github.com/meetqy/eagleuse/releases"} target="_blank">
      ...
      >
        ...
        <Typography.Text type="secondary">Copyright © {new Date().getFullYear()} - rao.pics</Typography.Text>
      </Layout.Footer>
    </Layout>
  );
};
